...
function woocommerce_bitpay_init()
{
    ...
    class WC_Gateway_Bitpay extends WC_Payment_Gateway
    {
        ...
        public function __construct()
        {
            ...
            $this->method_description = 'GloBee allows you to accept bitcoin payments on your WooCommerce store.';
            ...
            $this->log('GloBee Woocommerce payment plugin object constructor called. Plugin is v' . $this->debug_plugin_version . ' and server is PHP v' . $this->debug_php_version);
        ...
        public function is_valid_for_use()
        {
            ...
            try {
                ...
                if (false === isset($currency) || true === empty($currency)) {
                    $this->log('    [Error] The GloBee payment plugin was called to check if it was valid for use but could not instantiate a currency object.');
                    throw new \Exception('The GloBee payment plugin was called to check if it was valid for use but could not instantiate a currency object. Cannot continue!');
                }
            } catch (\Exception $e) {
        ...
        public function init_form_fields()
        {
            ...
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type'    => 'checkbox',
                    'label'   => __('Enable Bitcoin Payments via GloBee', 'bitpay'),
                    'default' => 'yes'
               ),
                'title' => array(
                ...
                'transaction_speed' => array(
                    ...
                    'type'        => 'select',
                    'description' => 'Choose a transaction speed.  For details, see the API documentation at globee.com',
                    'options'     => array(
                        'high'    => 'High',
                        'medium'  => 'Medium',
                        'low'     => 'Low',
                ...
                'notification_url' => array(
                    ...
                    'type'        => 'url',
                    'description' => __('GloBee will send IPNs for orders to this URL with the GloBee invoice data', 'bitpay'),
                    'default'     => '',
                ...
                'redirect_url' => array(
                    ...
                    'type'        => 'url',
                    'description' => __('After paying the GloBee invoice, users will be redirected back to this URL', 'bitpay'),
                    'default'     => '',
                ...
                'support_details' => array(
		            ...
		            'type'        => 'title',
		            'description' => sprintf(__('This plugin version is %s and your PHP version is %s. If you need assistance, please contact support@globee.com.  Thank you for using GloBee!', 'bitpay'), get_option('woocommerce_bitpay_version'), PHP_MAJOR_VERSION . '.' . PHP_MINOR_VERSION),
	           ),
           );
        ...
        public function process_payment($order_id)
        {
            ...
            if (false === empty($this->api_token)) {
                ...
                $this->log('    [Error] The GloBee payment plugin was called to process a payment but could not set client->setToken to this->api_token. The empty() check failed!');
                throw new \Exception(' The GloBee payment plugin was called to process a payment but could not set client->setToken to this->api_token. The empty() check failed!');
            }
        ...
        public function ipn_callback()
        {
            ...
            if (false === empty($this->api_token)) {
                ...
                $this->log('    [Error] The GloBee payment plugin was called to handle an IPN but could not set client->setToken to this->api_token. The empty() check failed!');
                throw new \Exception('The GloBee payment plugin was called to handle an IPN but could not set client->setToken to this->api_token. The empty() check failed!');
            }
        ...
        public function bitpay_encrypt($data)
        {
                ...
                strlen($fingerprint) > 24)
            {
                ...
                if (true === empty($encrypted)) {
                    throw new \Exception('The GloBee payment plugin was called to encrypt a serialized object and failed!');
                }
    ...
    **/
    ...
    function ajax_bitpay_pair_code()
    {
        ...
        if ( current_user_can( 'manage_options' ) ) {
            ...
            if (true === empty($key)) {
                throw new \Exception('The GloBee payment plugin was called to process a pairing code but could not instantiate a PrivateKey object. Cannot continue!');
            }
            ...
            if (true === empty($pub)) {
                throw new \Exception('The GloBee payment plugin was called to process a pairing code but could not instantiate a PublicKey object. Cannot continue!');
            }
            ...
            if (true === empty($sin)) {
                throw new \Exception('The GloBee payment plugin was called to process a pairing code but could not instantiate a SinKey object. Cannot continue!');
            }
            ...
            if (true === empty($client)) {
                throw new \Exception('The GloBee payment plugin was called to process a pairing code but could not instantiate a Client object. Cannot continue!');
            }
            ...
            if (true === empty($curlAdapter)) {
                throw new \Exception('The GloBee payment plugin was called to process a pairing code but could not instantiate a CurlAdapter object. Cannot continue!');
            }
    ...
    function bitpay_encrypt($data)
    {
            ...
            strlen($fingerprint) > 24)
        {
            ...
            if (true === empty($encrypted)) {
                throw new \Exception('The GloBee payment plugin was called to serialize an encrypted object and failed!');
            }
...
function woocommerce_bitpay_failed_requirements()
{
    ...
    if (true === empty($woocommerce)) {
        ...
        $errors[] = 'Your WooCommerce version is too old. The GloBee payment plugin requires WooCommerce 2.2 or higher to function. Your version is '.$woocommerce->version.'. Please contact your web server administrator for assistance.';
    }
    ...
    if (false === extension_loaded('curl')) {
        $errors[] = 'The GloBee payment plugin requires the Curl extension for PHP in order to function. Please contact your web server administrator for assistance.';
    }
